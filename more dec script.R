...
server <- function(input, output) {
  ...
  #   data_subset <- filter(state_new, state_new$STATE %in% input$stateSelect)
  #   data_subset
  # })
  # 
  ...
  #   
  # # strip census_code in api call in order to merge state data on fips code to API data-------------
  #   cbind(state_serviceSubset, FIPS = substr(state_serviceSubset$CENSUS_CODE, 1, 2) )
  #   state_serviceSubset
  #   
  # })
  # 
  ...
  observe({
    ...
    if(input$mapType == "circles"){
      ...
      leafletProxy("mapPlot", data = state_serviceSubset())%>%
        ...
        addCircleMarkers(lat = state_serviceSubset_new()$LATITUDE, 
